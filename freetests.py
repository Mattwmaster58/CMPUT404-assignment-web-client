...
class TestHTTPClient(unittest.TestCase):
    ...
    def testInternetGets(self):
        ...
        for url in urls:
            ...
            self.assertTrue(req.code == 200 or req.code == 301 or req.code == 302, "Code: %s for %s" % (req.code, url))
            if req.code == 200:
                self.assertTrue(
                    req.body.find("DOCTYPE") >= 0 or req.body.find("<body") >= 0, "%s Data: [%s] " % (url, req.body)
                )
